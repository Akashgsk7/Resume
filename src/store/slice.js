  ...
  reducers: {
    ...
    changeEmployment(state, action) {
      if (action.payload.delete) {
        ...
        let index = getIndex(state.employment, action.payload.data);
        state.employment.splice(index, 0, action.payload.data);

        return;
      }
      let index = getIndex(state.employment, action.payload);
    ...
    changeEductaion(state, action) {
      if (action.payload.delete) {
        ...
        let index = getIndex(state.education, action.payload.data);
        state.education.splice(index, 0, action.payload.data);

        return;
      }
      let index = getIndex(state.education, action.payload);
    ...
    changeURL(state, action) {
      if (action.payload.delete) {
        ...
        let words = action.payload.data.site.split(" "); // title case conversion
        let name = "";
      ...
      let words = action.payload.site.split(" "); // title case conversion
      let name = "";
